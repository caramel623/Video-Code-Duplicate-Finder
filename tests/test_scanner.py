...
def test_scanner_groups_cd_parts_by_base_code(tmp_path: Path):
    (tmp_path / "ABC-123-CD1.mp4").write_bytes(b"one")
    (tmp_path / "ABC-123-CD2.mp4").write_bytes(b"two")
    items = scan_videos(tmp_path)
    assert [(item.code, item.part) for item in items] == [("ABC-123", 1), ("ABC-123", 2)]
